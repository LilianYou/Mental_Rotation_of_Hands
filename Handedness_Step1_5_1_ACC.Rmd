---
title: "Handedness_Step1_5_1_ACC"
output: pdf_document
---
```{r}
library(readr)
...
```{r}
Lat2_acc_Interaction<-lsmeans(SLR_Lat2_Strength_acc, ~(Lat2))
Lat2_acc_Interaction
```
...
p_LT <-ggplot(data = LT,aes(x = Strength, color = as.factor(SLR), group = SLR, y = ACC)) +
  ...
  theme(text=element_text(face="bold", size=20), legend.position = "none", axis.title.x=element_blank())+ annotate("text", x = 2, y = 0.97, label = "*", color = "black", size = 16)
...
Figure6a <- ggarrange(p_LT, p_RT, widths = c(0.8, 1),
                ncol = 2, nrow = 1)


Figure6a

ggsave("6a.png", width = 10, height = 6)
```
...
```{r}
Strength_Lat2_Interaction<-lsmeans(SLR_Lat2_Strength_version_acc, ~(Lat2*Strength))
Strength_Lat2_Interaction
```

```{r}
pairs(Strength_Lat2_Interaction)
```
...
```{r}
SLR_Lat2_Strength_Strategy_Interaction<-lsmeans(SLR_Lat2_Strength_Strategy_acc, ~(Lat2*Strength*Strategy))
