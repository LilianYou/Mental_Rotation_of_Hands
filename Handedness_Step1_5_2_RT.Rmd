...
descriptive stats
```{r}
Logfile <- read_csv("~/Desktop/Handedness/Data_new/Step1/SLR_GestureType_Palm_Palm.csv")
View(Logfile)
str(Logfile)
summary(Logfile)
```
...
```{r}
SLR_Strength_rt_Interaction<-lsmeans(SLR_Lat2_Strength_rt, ~SLR|(Strength))
SLR_Strength_rt_Interaction
```

```{r}
pairs(SLR_Strength_rt_Interaction)
```
...
p_LT <-ggplot(data = LT,aes(x = Strength, color = as.factor(SLR), group = SLR, y = RT)) +
  ...
  theme(text=element_text(face="bold", size=20), legend.position = "none", axis.title.x=element_blank()) + annotate("text", x = 1, y = 4100, label = "*", color = "black", size = 16)
...
Figure6b <- ggarrange(p_LT, p_RT, widths = c(0.8, 1),
                ncol = 2, nrow = 1)


Figure6b

ggsave("6b.png", width = 10, height = 6)
```
...
```{r}
SLR_Lat2_Strength_rt_Interaction<-lsmeans(SLR_Lat2_Strength_version_rt, ~SLR|(Lat2*Strength))
SLR_Lat2_Strength_rt_Interaction
```
...
```{r}
SLR_Lat2_Strength_version_rt_Interaction<-lsmeans(SLR_Lat2_Strength_version_rt, ~SLR|(Lat2*Strength*version))
...
```{r}
SLR_Lat2_Strength_Strategy_Interaction<-lsmeans(SLR_Lat2_Strength_Strategy_rt, ~SLR|(Lat2*Strength*Strategy))
SLR_Lat2_Strength_Strategy_Interaction
```

```{r}
pairs(SLR_Lat2_Strength_Strategy_Interaction)
```